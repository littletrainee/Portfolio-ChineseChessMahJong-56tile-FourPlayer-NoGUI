...
void SetUp(Player &player1, Player &player2, Player &player3, Player &player4,
...
class PrintAll {
	public:
		...
		void PrintAll_SetNameAndMap(string player1, string player2, string player3,
																string player4, string wall, string river, 
		...
		void PrintAll_Print(Player player1, Player player2, Player player3, 
				Player player4,	vector<string> wallhand, vector<string> riverhand) {
			// print wall
			cout << "============================================================" << endl;
			...
			cout << "============================================================" << endl;
			PrintMeldHand(4, player4.meldhand);
			cout << "------------------------------------------------------------" << endl;
			PrintHand(4, player4.hand, false);
			cout << "============================================================" << endl;
		};
	private:
		// declare type
...
void ContinueToBookmakerOrChange(int &continuetobookmaker, int winner,
														     bool &player1bookmaker, bool &player2bookmaker,
																 bool &player3bookmaker, bool &player4bookmaker, 
	...
	if (winner == tempbookmaker){
		...
		switch (tempbookmaker){
			...
			case 3:
				...
				break;
			case 4:
				player4bookmaker = !player4bookmaker;
				gameturn = 3;
				break;
		}
...
void DiscardAndPrintHand(vector<string> sortby, map<string,string> tile, 
	...
	pa.PrintAll_SetNameAndMap(player1.name, player2.name, player3.name, 
			player4.name, "牌山", "牌河", tile);
	...
	// print all hand
	pa.PrintAll_Print(player1, player2, player3, player4, wallhand, riverhand);
}

// check win or meld function
void WinAndMeldCheck(vector<string> sortby, map<string,string> tile,
										 vector<string> &riverhand, vector<string> &wallhand, 
										 Player &player, int &gameturn, bool &gameloop,
										 bool nextplayer, bool &top, bool &winonthewalltail,
										 bool &tsumo, string &eye, bool &gameflow){
	// declare class
	bool ismeld = false;
	CheckWin cw;
	CheckMeld cm;
	...
	// player isn't ron then check meld
		...
		if (ismeld && !player.tenpai){
			...
			if (player.concealed && !player.meldhand.empty())
				...
				player.concealed = !player.concealed;
...
int main() {
	...
	bool top = true;
	// int
	int gameturn = 2; // #0 = player1, #1 = player2, #2 = player3, #3 = player4
	int gameround = 1;
	...
	s.SetTile(tile);
ReStart:
	SetUp(player1, player2, player3, player4, tile, wallhand, sortby);
	// print all hand
	pa.PrintAll_Print(player1, player2, player3, player4, wallhand, riverhand);
	...
	switch(gameturn){
		...
		// player4
		case 3:
			gameloop = cw.CheckWin_Check(player4, riverhand);
			break;
	}
	...
	// first player isn't winning
		...
		switch(gameturn){
			...
			case 2:
				...
				break;
			case 3:
				CheckKongTwoTime(sortby, tile, player4, wallhand,
						riverhand, gameloop, top, winonthewalltail, tsumo, eye);
				break;
		}
	}
Head:
	while (gameloop) {
		...
		gameturn != 3 ? gameturn += 1 : gameturn -= 3;
		// player4 discard and other palyer check tsumo, ron or meld
		if (gameturn == 0) {
			// player4 discard and printhand
			DiscardAndPrintHand(sortby, tile, gameround, riverhand, wallhand, player4, player1,
				player2, player3);
			winnerortempgameturn = gameturn;	
			// player3 win and meld check
			WinAndMeldCheck(sortby, tile, riverhand, wallhand, player3, gameturn, 
				gameloop,	false, top, winonthewalltail, tsumo, eye, gameflow);
			if (gameloop && winnerortempgameturn == gameturn){
				...
				if(gameloop && winnerortempgameturn == gameturn){
					...
					WinAndMeldCheck(sortby, tile, riverhand, wallhand, player1, gameturn, 
						gameloop,	true, top, winonthewalltail, tsumo, eye, gameflow);
				}
			}
			// print all hand
			pa.PrintAll_Print(player1, player2, player3, player4, wallhand, riverhand);
		// player1 discard and other palyer check tsumo, ron or meld
			...
			DiscardAndPrintHand(sortby, tile, gameround, riverhand, wallhand, player1, player2,
				player3, player4);
			winnerortempgameturn = gameturn;	
			// player4 win and meld check
			WinAndMeldCheck(sortby, tile, riverhand, wallhand, player4, gameturn,
				gameloop,	false, top, winonthewalltail, tsumo, eye, gameflow);
			if (gameloop && winnerortempgameturn == gameturn){
				// player3 win and meld check
				WinAndMeldCheck(sortby, tile, riverhand, wallhand, player3, gameturn,
			...
			// print all hand
			pa.PrintAll_Print(player1, player2, player3, player4, wallhand, riverhand);
		// player2 discard and other palyer check tsumo, ron or meld
		} else if (gameturn == 2) {
			// player2 discard and printhand
			DiscardAndPrintHand(sortby, tile, gameround, riverhand, wallhand, player2, player3,
				player4, player1);
			winnerortempgameturn = gameturn;	
			// player1 win and meld check
			WinAndMeldCheck(sortby, tile, riverhand, wallhand, player1, gameturn,
			...
			// print all hand
			pa.PrintAll_Print(player1, player2, player3, player4, wallhand, riverhand);
		// player3 discard and other palyer check tsumo, ron or meld
		} else if (gameturn == 3) {
			// player3 discard and printhand
			DiscardAndPrintHand(sortby, tile, gameround, riverhand, wallhand, player3, player4,
				player1, player2);
			winnerortempgameturn = gameturn;	
			// player2 win and meld check
			WinAndMeldCheck(sortby, tile, riverhand, wallhand, player2, gameturn,
	...
	switch (gameturn) {
		...
		case 2:
			s.SetAccordingTo(sortby, player3, riverhand, eye, gameround, gameturn,
					tsumo, lastwalltile, winonthewalltail,
					continuetobookmaker);
			winnerortempgameturn = 3;
			break;
		case 3:
			s.SetAccordingTo(sortby, player4, riverhand, eye, gameround, gameturn,
					tsumo, lastwalltile, winonthewalltail,
					continuetobookmaker);
			winnerortempgameturn = 4;
			break;
	}
	s.PrintScore();
	if (player4.bookmaker && winnerortempgameturn != 4) {
		return  0;
	}else{
		for(int i = 6; i > 0; i--){
			cout << i << "秒後開始下一輪" << endl; 
		...
		ContinueToBookmakerOrChange(continuetobookmaker, winnerortempgameturn, player1.bookmaker, 
				player2.bookmaker, player3.bookmaker, player4.bookmaker, tempbookmaker, gameturn);
		tempbookmaker = 0;
